...
left join employees on salary.id = employees.id
...
left join salary on salary.id = employees.id
...
select monthly_salary from salary
right join roles on roles.id = salary.id
...
full join salary on salary.id = employees.id
order by salary;
